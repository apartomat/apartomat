...
func (l *UserLoader) Load(key int) (*store.User, error) {
...
func (l *UserLoader) LoadThunk(key int) func() (*store.User, error) {
	l.mu.Lock()
...
func (l *UserLoader) LoadAll(keys []int) ([]*store.User, []error) {
	results := make([]func() (*store.User, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	users := make([]*store.User, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *UserLoader) LoadAllThunk(keys []int) func() ([]*store.User, []error) {
	results := make([]func() (*store.User, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *UserLoader) Prime(key int, value *store.User) bool {
	l.mu.Lock()
	var found bool
...
func (l *UserLoader) unsafeSet(key int, value *store.User) {
	if l.cache == nil {
		l.cache = map[int]*store.User{}
	}
	l.cache[key] = value
}

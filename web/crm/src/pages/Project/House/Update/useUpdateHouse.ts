...
export function useUpdateHouse(): [UpdateHouse, UpdateHouseMutationResult] {
    const client = useApolloClient()
    const [update, result] = useUpdateHouseMutation({ client, errorPolicy: "all" })

    return [(houseId: string, data: UpdateHouseInput) => update({ variables: { houseId, house: data } }), result]
}

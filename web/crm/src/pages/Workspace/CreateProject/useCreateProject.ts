...
export type Result = Initial | Creating | Done | Failed
...
export function useCreateProject(): [CreateProjectFn, CreateProjectMutationResult, Result] {
    const client = useApolloClient()
    const [create, result] = useCreateProjectMutation({ client, errorPolicy: "all" })

    const state = new Proxy<Result>(
        { state: State.INITIAL },
        {
                ...
                switch (name) {
                    case "state":
                        if (!result.called) {
                            ...
                            !result.loading &&
                            (result.error ||
                                (result.data && result.data?.createProject.__typename !== "ProjectCreated"))
                        ) {
                            return State.FAILED
                        } else if (
                            result.called &&
                            !result.loading &&
                            result.data &&
                            result.data?.createProject.__typename === "ProjectCreated"
                        ) {
                            return State.DONE
                        }
    ...
    return [
        ({ workspaceId, name, startAt, endAt }) =>
            create({ variables: { input: { workspaceId, name, startAt, endAt } } }),
        result,
        state as Result,
    ]
}

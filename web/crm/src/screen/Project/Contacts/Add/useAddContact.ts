...
export function useAddContact(): [AddContact, AddContactMutationResult] {
    const client = useApolloClient()
    const [add, result] = useAddContactMutation({ client, errorPolicy: "all" })

    return [(projectId: string, contact: AddContactInput) => add({ variables: { projectId, contact } }), result]
}

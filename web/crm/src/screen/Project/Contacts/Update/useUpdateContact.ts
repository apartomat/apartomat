...
export function useUpdateContact(): [UpdateContact, UpdateContactMutationResult] {
    const client = useApolloClient()
    const [update, result] = useUpdateContactMutation({ client, errorPolicy: "all" })

    return [(contactId: string, data: UpdateContactInput) => update({ variables: { contactId, data } }), result]
}
